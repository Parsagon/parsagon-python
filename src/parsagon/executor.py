...
class Executor:
    ...
    def __init__(
        ...
        self.camoufox = Camoufox(main_world_eval=True, headless="virtual" if headless else False)
        self.driver = camoufox.__enter__()
        self.context = self.driver.new_context()
        self.parsagon_window = None
        ...
        self.context.add_init_script(path=highlights_path)
        # with highlights_path.open() as f:
        #    self.highlights_script = "mw:" + f.read()
    ...
    def inject_highlights_script(self):
        # self.parsagon_window.evaluate(self.highlights_script)
        pass

    def highlights_setup(self, field_type, max_examples="null"):
        self.parsagon_window.evaluate(
            f"mw:window.currentFieldType = '{field_type}'; window.maxExamples = {max_examples};"
        )

    def highlights_cleanup(self):
        self.parsagon_window.evaluate(
            f"mw:window.currentFieldType = null; window.maxExamples = null; window.clearCSS();"
        )

    def get_selected_node_ids(self, css_selector=None, xpath_selector=None):
        if css_selector:
            return [elem.get_attribute("data-psgn-id") for elem in self.parsagon_window.locator(css_selector).all()]
        elif xpath_selector:
            return [elem.get_attribute("data-psgn-id") for elem in self.parsagon_window.locator(xpath_selector)]
        else:
            return [
                elem.get_attribute("data-psgn-id")
                for elem in self.parsagon_window.locator(".parsagon-io-example-stored")
            ]

    def get_selected_node_and_descendant_ids(self):
        return self.parsagon_window.evaluate(
            "Array.from(document.getElementsByClassName('parsagon-io-example-stored')).map((elem) => [elem, ...elem.querySelectorAll('*')]).flat().map((elem) => elem.getAttribute('data-psgn-id'))"
        )
    ...
    def mark_html(self):
        ...
        """
        max_elem_id = self.max_elem_ids[id(self.parsagon_window)]
        max_elem_id = self.parsagon_window.evaluate(
            f"mw:() => {{ let elemIdx = {max_elem_id}; "
        ...
        self.max_elem_ids[id(self.parsagon_window)] = max_elem_id
        self.parsagon_window.evaluate(
            "mw:() => { for (const image of document.images) { image.setAttribute('data-psgn-width', image.parentElement.offsetWidth ?? -1); image.setAttribute('data-psgn-height', image.parentElement.offsetHeight ?? -1); } }"
        )

    def _get_cleaned_lxml_root(self):
        parser = lxml.html.HTMLParser(remove_comments=True, remove_pis=True)
        root = lxml.html.fromstring(self.parsagon_window.content().replace("&nbsp;", " "), parser=parser)

        # make links absolute
        root.make_links_absolute(self.parsagon_window.url)
        for elem in root.xpath("//img[@srcset]"):
            ...
            for s in elem.get("srcset").split(","):
                ...
                if not parts:
                    continue
                parts[0] = urljoin(self.parsagon_window.url, parts[0])
    ...
    def get_visible_html(self):
        ...
        assert "data-psgn-id" in self.parsagon_window.content()
        root = self._get_cleaned_lxml_root()
        ...
        visible_elem_ids = set(
            self.parsagon_window.evaluate(
                "Array.from(document.getElementsByTagName('*')).filter((elem) => { const style = getComputedStyle(elem); const rect = elem.getBoundingClientRect(); return style.opacity > 0.1 && style.display !== 'none' && style.visibility === 'visible' && elem.offsetWidth && elem.offsetHeight && elem.getClientRects().length && rect.right + window.scrollX >= 0 && rect.bottom + window.scrollY >= 0 }).map((elem) => elem.getAttribute('data-psgn-id'))"
            )
        )
        max_elem_id = self.max_elem_ids[id(self.parsagon_window)]
    ...
    def _id_to_elem(self, elem_id):
        ...
        result = self.parsagon_window.locator(f'//*[@data-psgn-id="{elem_id}"]')
        return result
    ...
    def goto(self, url, window_id=None):
        if window_id in self.driver.pag2s:
            self.parsagon_window = window_id
        else:
            self.parsagon_window = self.context.new_page()

        # Go to website
        browser_print(f"Going to {url}")
        self.parsagon_window.goto(url)

        # Wait for website to load
        parsagon_window.wait_for_timeout(1000)
        self.mark_html()
        self.inject_highlights_script()

        return self.parsagon_window

    def goto_lite(self, url, window_id=None):
        if window_id in self.driver.pages:
            self.parsagon_window = window_id
        else:
            self.parsagon_window = self.context.new_page()
        ...
        self.parsagon_window.goto("data:text/html;base64," + base64.b64encode(html).decode())
        parsagon_window.wait_for_timeout(1000)
        return self.parsagon_window

    def close_window(self, window_id):
        window_id.close()
        self.parsagon_window = self.driver.pages[-1]

    def _click_elem(self, elem, window_id):
        try:
            window_id.evaluate("elem => elem.click()", elem)
            browser_print("Clicked element")
            window_id.wait_for_timeout(2000)
    ...
    def _select_option(self, elem, option, window_id):
        for i in range(3):
            try:
                elem.select_option(label=option)
    ...
    def get_inner_text(self, window_id):
        return window_id.evaluate("document.body.innerText")
    ...
    def get_inner_markdown(self, window_id):
        html = window_id.evaluate("document.body.innerHTML")
    ...
    def get_pdf_text(self, url):
        window_id = self.context.new_page()
        with window_id.expect_download(timeout=30 + delay) as download_info:
            self.goto(url, window_id=window_id)
        download = download_info.value
        download.save_as(f"./download.pdf")
        self.close_window(window_id)
        files = glob.glob("./download.pdf")
        if len(files) != 1:
            return None
        reader = PdfReader(new_files[0])
        text = ""
